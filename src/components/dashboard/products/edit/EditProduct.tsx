...
import { getProduct, updateProduct } from "../../../../api/services/products";
...
const EditProduct = () => {
  const { id } = useParams();
  const productId = useRef<number>(Number(id));
    ...
    if (id) {
      getProduct(+id)
        .then((res) => {
          ...
          fillProductEditForm(setValue, res.data);
          setImageFile(res.data?.productImage);
          updateSavedVariant(res.data);
        })
        .catch((error) => {
          console.log(error);
        });
  ...
  const updateSavedVariant = (product: Product) => {
  ...
  const onSubmit = async (data: ProductEditFormFields) => {
    try {
      const formFiled = {
        ...data,
      ...
      await updateProduct(productId.current, formFiled);
      navigate(`/dashboard`);
    } catch (error) {
              ...
              <Row className="py-3">
                ...
                <Col md="6" className="px-2">
                  <Button
                    ...
                    type="submit"
                  >
                    <span className="text-center text-white">Update</span>
                  </Button>
                </Col>
              </Row>
            </Col>
